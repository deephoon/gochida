...
| 영역 | 상태 | 현재 구현 내용 |
|---|---|---|
| 사진 기반 요청 접수 | 완료 | 카메라·갤러리 최대 3장, 위치·증상 선택, 입력 검증 |
| AI 요청서 작성 | 완료 | Gemini 2.5 Flash 멀티모달 분석, Mock fallback, 응답 정규화·길이 제한 |
| 요청서 검토 | 완료 | 구조화된 요청 내용 확인, 추가 메모 전역 보관 |
| 전문가 비교·상세 | 완료 | 조건 비교, 신뢰 지표, 작업 확인서·안심 보증서 미리보기 |
| 상담 준비·채팅 | 로컬 Mock 완료 | 첫 메시지 생성, 상담방 생성, 메시지 전송과 탭 이동 후 상태 유지 |
| 실제 백엔드 연동 | 예정 | 인증, 요청 이력, 전문가 매칭, 실시간 채팅, 푸시 알림 |

현재 앱은 **사용자 요청 시작 → AI 분석 → 요청서 검토 → 전문가 비교 → 상담 준비 → 1:1 채팅**까지
전체 핵심 플로우를 실행할 수 있습니다. 전문가 응답과 채팅은 로컬 Mock 데이터 및
`RequestContext` 상태를 사용하며, 서버 저장과 실시간 통신은 다음 단계입니다.

## 🌿 개발 워크플로우

이 프로젝트는 안정적인 `main`과 짧게 유지하는 Topic 브랜치를 중심으로 한 **GitHub Flow**를
사용합니다. Expo 모바일 배포 버전은 `main`의 Git 태그로 관리하고, 긴급 배포 수정에만
`hotfix/*` 브랜치를 사용합니다.

- 작업 전 GitHub Issue 생성
- `main`에서 `feature/<issue>-<slug>`, `fix/<issue>-<slug>` 등 목적별 브랜치 생성
- Conventional Commit 형식으로 커밋하고 작업 중에도 지속적으로 push
- Draft PR에서 리뷰와 `npm run lint`, `npx tsc --noEmit` 검증
- 검증된 변경만 `main`에 Squash merge하고 배포 시 `vX.Y.Z` 태그 생성

세부 규칙과 명령어는 [Gochida 브랜치 전략](docs/BRANCH_STRATEGY.md)을 참고하세요. 전략은
[Git 브랜치 전략 (feat. Git Flow, Github Flow)](https://hudi.blog/git-branch-strategy/)의
GitHub Flow 원칙을 현재 프로젝트 운영 방식에 맞게 적용했습니다.
...
1. [현재 구현 상태](#-현재-구현-상태)
2. [개발 워크플로우](#-개발-워크플로우)
3. [프로젝트 목적 및 배경](#-프로젝트-목적-및-배경)
4. [AX/DX 전략](#-axdx-전략)
5. [핵심 기능 상세](#-핵심-기능-상세)
6. [화면별 상세 플로우](#-화면별-상세-플로우)
7. [아키텍처 설계](#-아키텍처-설계)
8. [AI 프롬프트 엔지니어링](#-ai-프롬프트-엔지니어링)
9. [디자인 시스템](#-디자인-시스템)
10. [컴포넌트 라이브러리](#-컴포넌트-라이브러리)
11. [데이터 모델 및 타입 시스템](#-데이터-모델-및-타입-시스템)
12. [프로젝트 구조](#-프로젝트-디렉토리-구조)
13. [기술 스택](#-기술-스택)
14. [시작하기](#-시작하기-getting-started)
15. [Expo 앱 설정](#-expo-앱-설정-appjson)
16. [참고 시세 데이터](#-참고-시세-데이터)
17. [지표 및 로깅 전략](#-지표-및-로깅-전략)
18. [보안 및 주의사항](#-보안-및-주의사항)
19. [향후 개선 방향](#-향후-개선-방향-roadmap)
...
- **이미지 품질 최적화**: 카메라는 `quality: 0.6`, 갤러리는 `quality: 0.7`로 압축하여 API 전송 용량을 줄이면서도 분석에 충분한 화질을 유지합니다.
...
3. **상담/의뢰하기 CTA**
   - 하단 플로팅 푸터(`floatingFooter`)에 위치하며, 약간의 투명도가 있는 배경 위로 섀도우가 적용된 CTA 버튼이 제공됩니다. 탭 시 상담 준비 화면(`/consultation/[expertId]`)으로 이동합니다.
...
│   │   ├── consultation/
│   │   │   └── [expertId].tsx             # 6단계: 요청 요약·첫 메시지 확인 후 상담 시작
│   │   └── chat/
│   │       └── [id].tsx                   # 7단계: 상담방 메시지 조회·전송
│   │
...
│   │   ├── CertificatePreviewCard.tsx      # 작업 확인서 미리보기 카드
│   │   ├── Chip.tsx                       # Pill 형태의 토글 선택 버튼 (햅틱 피드백 적용)
│   │   ├── ProcessBottomNav.tsx            # 요청 진행 화면 공용 하단 내비게이션
│   │   ├── ProcessCta.tsx                  # 요청 진행 화면 공용 CTA
│   │   ├── Skeleton.tsx                   # 투명도 애니메이션 자리표시자
│   │   └── TrustStatsCard.tsx              # 전문가 신뢰 지표 카드
│   │
│   ├── context/
│   │   └── RequestContext.tsx             # 요청·추가 메모·상담방 전역 상태 관리
│   │
...
│
├── docs/
│   ├── BRANCH_STRATEGY.md                 # GitHub Flow 기반 브랜치·커밋·PR·릴리스 규칙
│   └── PRD.md                             # 제품 요구사항 정의서 (AX/DX 전략, MoSCoW, 지표)
│
├── .github/
│   └── pull_request_template.md            # 변경 이유·검증·이슈 연결을 위한 PR 템플릿
│
├── ref/                                   # 참고 자료
...
| **`.env` 파일** | `.gitignore`에서 `.env`와 `.env*.local` 제외 | `.env.example`에는 비밀값 없이 키 이름만 유지 |
...
- [ ] GitHub Actions에 `lint`·`typecheck` 검사 추가 및 `main` 보호 규칙 적용

### 🟡 Should Have (서비스 완성도)
- [x] 카메라 직접 촬영 기능 (`expo-image-picker` 카메라 모드 — `useRequestFlow.takePhoto()`로 구현 완료)
- [x] 사용자-전문가 상담 준비 및 로컬 Mock 채팅 (`RequestContext` 기반 상태 유지)
- [ ] 실제 전문가 매칭 백엔드 연동
- [ ] 실시간 채팅 백엔드 및 영구 저장 연동
